        ...
        listViewDeepAnse.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    if (adapterViewByMonth.isViewSelected()) {
                        adapterViewByMonth.setColorSelectedView(getResources().getColor(R.color.ColorBlue));
                    }
                } else if (event.getAction() == MotionEvent.ACTION_MOVE) {
                    if (!adapterViewByMonth.isViewSelected()) {
                        adapterViewByMonth.removeSelectedView();
                    }
                } else {
                    adapterViewByMonth.removeSelectedView();
                }
